...
<#
  MailGuard - assign the Exchange read-only role (One-Click sign-in app)

  Run this AFTER MailGuard-OAuth-Setup.ps1, and after you have connected via
  Connect > One-Click OAuth and MailGuard shows the tenant connected. It reads the
  values the setup script saved (mailguard-signin-credentials.txt in this same
  folder), or prompts for them, then grants the sign-in app the read-only Exchange
  role so MailGuard can read mail-flow and transport data.

  One-Click already assigns an Exchange directory role automatically during sign-in;
  run this only if Exchange stays "pending" after ~30 minutes. It targets the same
  'MailGuard Sign-In' app the One-Click flow uses - not the Manual 'MailGuard Scanner' app.
  ...
  HOW TO RUN (run as a FILE - do not paste the lines one by one):
    ...
    Mac:      install PowerShell once:  brew install --cask powershell
              then run:  pwsh ./MailGuard-OAuth-ExchangeRole.ps1
    Linux:    install PowerShell 7, then run:  pwsh ./MailGuard-OAuth-ExchangeRole.ps1
#>
...
$credFile = Join-Path (Get-Location) 'mailguard-signin-credentials.txt'
